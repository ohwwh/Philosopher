...
static void	sems_init(int num, t_info *sh_info)
{
	int		i;
	char	*str;

	i = 0;
	sh_info->fork = sem_open("forks", O_CREAT, S_IRWXU, num);
	sem_unlink("forks");
	sh_info->deadlock_check = sem_open("deadlock", O_CREAT, S_IRWXU, (num / 2));
	sh_info->print = sem_open("print", O_CREAT, S_IRWXU, 1);
	sem_unlink("deadlock");
	sem_unlink("print");
	...
	{
		str = ft_itoa(i + 1);
		sh_info->end_eat[i] = sem_open(str, O_CREAT, S_IRWXU, 0);
		sem_unlink(str);
		free(str);
		str = ft_itoa(i + 201);
		sh_info->sim_start[i] = sem_open(str, O_CREAT, S_IRWXU, 0);
		sem_unlink(str);
		free(str);
		i ++;
	}
